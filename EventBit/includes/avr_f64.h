...
// -2^^31, f_float64_to_long(x) yields the correct value. If the absolute value of x is 2^^32 or greater,
// f_float64_to_long(x) returns zero. ATTENTION: If -2^^32 < x < -2^^31, f_float64_to_long(x) returns
// 2^^32 plus the integer fraction of x.
...
int8_t f_isnan(float64_t x);	// Returns nonzero if x is an IEEE 754 "Not A Number" and otherwise zero.
int8_t f_finite(float64_t x);	// Returns nonzero if x represents a real number and zero otherwise i.e. if
	// x is +INF, -INF or NaN.
int8_t f_compare(float64_t x, float64_t y); // If both x and y represent real numbers
...
float64_t f_strtod(char *str, char **endptr); // Converts a decimal representation of a real number
...
float64_t f_sqrt(float64_t x);	// Returns the square root of x if x is nonnegative and NaN otherwise.
float64_t f_exp(float64_t x);	// Evaluates the exponential function e^x at x (where e is Euler's constant).
float64_t f_log(float64_t x);	// Returns the logarithm of x if x is positive or NaN if x is negative.
	// If x is zero, f_log() will return -INF if F_ONLY_NAN_NO_INFINITY is not defined and NaN otherwise.
#define f_pow(x, y) (f_exp(f_mult((y), f_log(x))))	// Returns the power x^y if x is positive or NaN if
	// x is negative. If x is zero, f_pow(0,y) will return NaN if F_ONLY_NAN_NO_INFINITY is defined or
	// if y is zero; If x is zero, y is nonzero and F_ONLY_NAN_NO_INFINITY is not defined,
	// f_pow(x,y) returns zero. Special cases: pow(0, INF)=0, pow(0, -INF)=+INF, pow(1, +/-INF)=NaN
	// pow(x, INF)={ 0 if 0<=x<1 and INF if x>1 }
	// pow(x, -INF)={ +INF if 0<=x<1 and 0 if x>1 }
	// pow(+INF, 0)=NaN, pow(+INF, x)=+INF if x!=0
...
#define f_arcsin(x) (f_berechne_arcsin_arccos_arctan((x), 0))	// Returns the arc sine of x if -1<=x<=1 and NaN otherwise
#define f_arccos(x) (f_berechne_arcsin_arccos_arctan((x), 1))	// Returns the arc cosine of x if -1<=x<=1 and NaN otherwise
